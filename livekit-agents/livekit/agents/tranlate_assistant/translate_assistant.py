...
WillSynthesizeAssistantReply = Callable[
    ["TranslateAssistant", ChatContext],
...
_CallContextVar = contextvars.ContextVar["AssistantCallContext"](
    "translate_assistant_contextvar"
)


class AssistantCallContext:
    def __init__(self, assistant: "TranslateAssistant", llm_stream: LLMStream) -> None:
...
def _default_will_synthesize_assistant_reply(
    assistant: TranslateAssistant, chat_ctx: ChatContext
) -> LLMStream:
...
class TranslateAssistant(utils.EventEmitter[EventTypes]):
    def __init__(
        ...
        *,
        ...
        self._human_input_map: dict[str, HumanInput] = dict()
        self._agent_output_map: dict[str, AgentOutput] = dict()
        self._track_published_fut_map = dict[str, asyncio.Future[None]] = dict()

        self._agent_answer_speech_map: dict[str, _SpeechInfo] = dict()
        self._agent_playing_speech_map: dict[str, _SpeechInfo] = dict()
        self._agent_answer_atask_map: dict[str, asyncio.Task[None]] = dict()
        self._playout_ch = utils.aio.Chan[_SpeechInfo]()

        self._transcribed_text_map: dict[str, str] = dict()
        self._transcribed_interim_text_map: dict[str, str] = dict()

        self._deferred_validation_map: dict[str, _DeferredAnswerValidation] = dict()
    ...
    async def aclose(self) -> None:
        ...
        if self._deferred_validation_map is not None:
            for _, item in self._deferred_validation_map.items():
                await item.aclose()

    def _on_participant_connected(self, participant: rtc.RemoteParticipant):
        language = self._get_language_by_participant(participant)
        if self._human_input_map[language] is not None:
            return

        self._link_participant(participant.identity)

    def _get_language_by_participant(self, participant: rtc.RemoteParticipant) -> str:
        metadata_dict = json.loads(participant.metadata)
        return metadata_dict.get("language", "disabled")
    ...
    def _link_participant(self, identity: str) -> None:
        ...
        language = self._get_language_by_participant(participant)
        self._track_published_fut_map[language] = asyncio.Future[None]()
        self._deferred_validation_map[language] = _DeferredAnswerValidation(
            self._validate_answer_if_needed, loop=self._loop, language=language
        )
        self._add_audio_track(language)

        self._human_input_map[language] = HumanInput(
            room=self._room,
            vad=self._vad,
            stt=self._stt,
        ...
        def _on_start_of_speech(ev: vad.VADEvent) -> None:
            ...
            self._deferred_validation_map[language].on_human_start_of_speech(ev)

        def _on_vad_updated(ev: vad.VADEvent) -> None:
            if not self._track_published_fut_map[language].done():
                return

            assert self._agent_output_map[language] is not None
            tv = max(0, 1 - ev.probability)
            self._agent_output_map[language].audio_source.target_volume = tv
            smoothed_tv = self._agent_output_map[language].audio_source.smoothed_volume
            ...
            if ev.speech_duration >= self._opts.int_speech_duration:
                self._interrupt_if_needed(language)

        def _on_end_of_speech(ev: vad.VADEvent) -> None:
            self._plotter.plot_event("user_stopped_speaking")
            self.emit("user_stopped_speaking")
            self._deferred_validation_map[language].on_human_end_of_speech(ev)

        def _on_interim_transcript(ev: stt.SpeechEvent) -> None:
            self._transcribed_interim_text_map[language] = ev.alternatives[0].text

        def _on_final_transcript(ev: stt.SpeechEvent) -> None:
            self._transcribed_text_map[language] += ev.alternatives[0].text
            self._synthesize_answer(user_transcript=self._transcribed_text_map[language], language=language)

        self._human_input_map[language].on("start_of_speech", _on_start_of_speech)
        self._human_input_map[language].on("vad_inference_done", _on_vad_updated)
        self._human_input_map[language].on("end_of_speech", _on_end_of_speech)
        self._human_input_map[language].on("interim_transcript", _on_interim_transcript)
        self._human_input_map[language].on("final_transcript", _on_final_transcript)

    async def _add_audio_track(self, language: str) -> None:
        audio_source = rtc.AudioSource(self._tts.sample_rate, self._tts.num_channels)
        track = rtc.LocalAudioTrack.create_audio_track(f"translate_voice_{language}", audio_source)
        await self._room.local_participant.publish_track(
            track, rtc.TrackPublishOptions(source=rtc.TrackSource.SOURCE_MICROPHONE)
        )

        cancellable_audio_source = CancellableAudioSource(source=audio_source)
        self._agent_output_map[language] = AgentOutput(
            room=self._room,
        ...
        self._track_published_fut_map[language].set_result(None)
    ...
    async def _main_task(self) -> None:
        ...
        async for speech in self._playout_ch:
            self._agent_playing_speech_map[speech.language] = speech
            await self._play_speech(speech)
            self._agent_playing_speech_map[speech.language] = None

    def _validate_answer_if_needed(self, language: str) -> None:
        """
        Check if the user speech should be validated/played
        """
        if (
            self._agent_answer_speech_map[language] is None
            or self._agent_answer_speech_map[language].synthesis_handle.interrupted
        ):
            return

        self._playout_ch.send_nowait(self._agent_answer_speech_map[language])
        self._agent_answer_speech_map[language] = None

    def _interrupt_if_needed(self, language: str) -> None:
        """
        Check whether the current assistant speech should be interrupted
        """
        if (
            self._agent_playing_speech_map[language] is None
            or not self._agent_playing_speech_map[language].allow_interruptions
            or self._agent_playing_speech_map[language].synthesis_handle.interrupted
        ):
            return
        ...
        if self._opts.int_min_words != 0:
            ...
            interim_words = self._opts.word_tokenizer.tokenize(
                text=self._transcribed_interim_text_map[language]
            )
            if len(interim_words) < self._opts.int_min_words:
                return

        self._agent_playing_speech_map[language].synthesis_handle.interrupt()

    def _synthesize_answer(self, *, user_transcript: str, language: str):
        """
        ...
        async def _synthesize_answer_task(old_task: asyncio.Task[None]) -> None:
            ...
            assert (
                self._agent_output_map[language] is not None
...
{language}
"""
            user_msg = ChatMessage.create(text=input, role="user")
            ...
            synthesis = self.agent_output_map[language].synthesize(
                transcript=_llm_stream_to_str_iterable(llm_stream)
            )
            self._agent_answer_speech_map[language] = _SpeechInfo(
                source=llm_stream,
            ...
            self._deferred_validation_map[language].on_new_synthesis(user_transcript)

        if self._agent_answer_speech_map[language] is not None:
            self._agent_answer_speech_map[language].synthesis_handle.interrupt()

        self._agent_answer_speech_map[language] = None
        old_task = self._agent_answer_atask_map[language]

        self._agent_answer_atask_map[language] = asyncio.create_task(
    ...
    async def _play_speech(self, speech_info: _SpeechInfo) -> None:
        ...
        assert (
            self._agent_output_map[speech_info.language] is not None
        ), "agent output should be initialized when ready"
        ...
        def _commit_user_message_if_needed() -> None:
            ...
            self._transcribed_text_map[speech_info.language] = self._transcribed_text_map[speech_info.language][len(user_question) :]
        ...
        if is_using_tools and not interrupted:
            ...
            if tool_calls:
                ...
                answer_synthesis = self._agent_output_map[speech_info.language].synthesize(
...
class _DeferredAnswerValidation:
    ...
    def __init__(
        self, validate_fnc: Callable[[str], None], loop: asyncio.AbstractEventLoop, language: str
    ) -> None:
    ...
    async def _run_task(self, delay: float) -> None:
        ...
        self._validate_fnc(self.language)
